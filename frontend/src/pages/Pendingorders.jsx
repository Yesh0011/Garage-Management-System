...
export default function PendingOrders() {
  ...
  return (
    <div className="bg-white min-h-screen text-black p-4 rounded-lg shadow-md">
        ...
        <br /><br />
      <h1 className="text-3xl font-semibold mb-4">Pending Orders</h1>
        ...
        <tbody>
          {orders.map(order => (
            <tr key={order._id} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="py-2 text-wrap ml-10 mr-10">{order.description}</td>
              <td className="py-2 text-center">{new Date(order.deadline).toLocaleDateString()}</td>
              <td className="py-2 text-center">
                <div className="flex gap-2 justify-center">
                  <button onClick={() => handleDelete(order._id)} className="bg-red-600 text-white py-1 px-2 rounded-lg hover:bg-red-700 focus:outline-none">Delete</button>
